...
const TEST_CASES = [
  { file: "sudoku-01.jpg", expected: SUDOKU_01_EXPECTED },
  { file: "sudoku-04.jpg", expected: SUDOKU_01_EXPECTED }, // Same puzzle, iPhone photo
];

test.describe("OCR Accuracy", () => {
  for (const { file, expected } of TEST_CASES) {
    test(`${file} should meet accuracy threshold`, async ({ page }) => {
      await page.goto("/");

      // Open dropdown menu and click Capture puzzle
      await page.getByRole("button", { name: "Menu" }).click();
      await page.getByRole("button", { name: "Capture puzzle" }).click();

      // Wait for import modal
      await expect(page.getByText("Import Puzzle")).toBeVisible();

      // Upload fixture image via hidden file input
      const fileInput = page.locator('input[type="file"]');
      await fileInput.setInputFiles(join(FIXTURES_DIR, file));
      ...
      const results = compareResults(recognizedDigits, expected);

      console.log(`\n=== OCR Accuracy Results (${file}) ===`);
